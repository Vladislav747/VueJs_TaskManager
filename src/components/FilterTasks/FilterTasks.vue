        ...
        <div class="filter-inner">
            ...
            <div class="filter-property">
                ...
                <select class="filter-property--body" v-model="filterCategory">
                    <option></option>
                    <option v-for="type in taskTypes" :key="type">
                        {{ type }}
                    </option>
                </select>
            </div>
            ...
            <div class="filter-property">
                ...
                <select
                    ...
                    v-model="filterDateDeadline"
                >
                    <option></option>
                    <option v-for="type in deadlineTypes" :key="type">
                        {{ type }}
                    </option>
                </select>
            </div>
            ...
            <div class="clear-result__wrapper">
                <button
                    @click="clearResults()"
                    class="btn-task clear-results__btn bg-red-element"
                >
                    Сбросить результаты
                </button>
            </div>
...
export default {
    ...
    watch: {
        ...
        filterDateDeadline: function () {
            ...
            if (this.filteredTasksTime.length === 0) {
                filterData = this.tasks.slice(0);
                filterData.forEach((element) => {
                    element.cards = element.cards.filter(
    ...
    methods: {
        ...
        filterTaskDeadline(task) {
            const dateDeadline = new Date(task.dateOfTask).getTime();
         ...
         */
        showFilter() {
            const divFilter = document.getElementsByClassName(
                'filterWrapper'
            )[0];
            const overlay = this.$root.$data.showOverlay();
